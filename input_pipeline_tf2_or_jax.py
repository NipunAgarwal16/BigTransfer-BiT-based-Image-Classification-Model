...
def get_dataset_info(dataset, split, examples_per_class):
    ...
    return {
        "original_num_examples": original_num_examples,
...
def sample_subset(
    data, num_examples, num_classes, examples_per_class, examples_per_class_seed
):
    ...
    indices = [
        ...
        for idx in np.random.choice(
            np.where(data["label"] == c)[0], examples_per_class, replace=False
        )
    ]

    data = {"image": data["image"][indices], "label": data["label"][indices]}
    ...
    return data.map(
        lambda x, y: {"image": x, "label": y}, tf.data.experimental.AUTOTUNE
    )
