...
def reshape_for_keras(features, batch_size, crop_size):
    features["image"] = tf.reshape(
        features["image"], (batch_size, crop_size, crop_size, 3)
    )
...
def main(args):
    ...
    data_train = data_train.map(
        lambda x: reshape_for_keras(x, batch_size=args.batch, crop_size=crop_size)
    )
    ...
    for epoch, accu in enumerate(history.history["val_accuracy"]):
        logger.info(f"Step: {epoch * args.eval_every}, " f"Test accuracy: {accu:0.3f}")
