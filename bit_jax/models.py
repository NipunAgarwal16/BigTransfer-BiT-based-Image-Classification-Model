...
def fixed_padding(x, kernel_size):
    ...
    x = jax.lax.pad(
        x, 0.0, ((0, 0, 0), (pad_beg, pad_end, 0), (pad_beg, pad_end, 0), (0, 0, 0))
    )
    return x
...
class RootBlock(nn.Module):
    ...
    def apply(self, x, width):
        x = fixed_padding(x, 7)
        x = StdConv(
            x, width, (7, 7), (2, 2), padding="VALID", bias=False, name="conv_root"
        )
...
class ResNet(nn.Module):
    ...
    def apply(self, x, num_classes=1000, width_factor=1, num_layers=50):
        ...
        # Head
        x = nn.Dense(
            x, num_classes, name="conv_head", kernel_init=nn.initializers.zeros
        )
