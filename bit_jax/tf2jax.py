...
def transform_params(params, params_tf, num_classes):
    ...
    params["root_block"]["conv_root"]["kernel"] = params_tf[
    ...
    for block in ["block1", "block2", "block3", "block4"]:
        ...
        for unit in units:
            for i, group in enumerate(["a", "b", "c"]):
                params[block][unit][f"conv{i+1}"]["kernel"] = params_tf[
                    f"resnet/{block}/{unit}/{group}/" "standardized_conv2d/kernel"
                ]
                params[block][unit][f"gn{i+1}"]["bias"] = params_tf[
                    f"resnet/{block}/{unit}/{group}/" "group_norm/beta"
                ][None, None, None]
                params[block][unit][f"gn{i+1}"]["scale"] = params_tf[
    ...
    params["norm-pre-head"]["bias"] = params_tf["resnet/group_norm/beta"][
        None, None, None
    ]
    params["norm-pre-head"]["scale"] = params_tf["resnet/group_norm/gamma"][
        None, None, None
    ]
