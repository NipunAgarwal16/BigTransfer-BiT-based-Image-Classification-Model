...
def main(args):
    ...
    tf2jax.transform_params(params, params_tf, num_classes=dataset_info["num_classes"])
    ...
    # Run training loop
    for step, batch in zip(range(1, total_steps + 1), data_train.as_numpy_iterator()):
        lr = bit_hyperrule.get_lr(step - 1, dataset_info["num_examples"], args.base_lr)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tfds_manual_dir", default=None, help="Path to maually downloaded dataset."
    )
    parser.add_argument("--batch_eval", default=32, type=int, help="Eval batch size.")
